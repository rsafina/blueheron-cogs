...
function setDate() {
  ...
  document.getElementById('page-date').textContent =
    d.toLocaleDateString('en-ID', opts);
}
...
function initNav() {
  document.querySelectorAll('.nav-item').forEach(link => {
    link.addEventListener('click', e => {
      ...
      const titles = { dashboard:'Overview', production:'Production', stock:'Stock', purchases:'Purchases', recipes:'Recipes', items:'Item Master', suppliers:'Suppliers', service:'Service', adjustments:'Adjustments', ledger:'Ledger', costing:'Costing', calculator:'Calculator' };
...
async function loadPage(page) {
  switch (page) {
    ...
    case 'production':  return loadProduction();
    case 'stock':       return loadStock();
    case 'purchases':   return loadPurchases();
    case 'recipes':     return loadRecipes();
    case 'items':       return loadItems();
    case 'suppliers':   return loadSuppliers();
    case 'service':     return loadService();
    case 'adjustments': return loadAdjustments();
    case 'ledger':      return loadLedger();
    case 'costing':     return loadCosting();
    case 'calculator':  return loadCalculator();
  }
}
...
function renderCapacityList(portions) {
  ...
  if (!portions.length) { el.innerHTML = '<div class="loading-row">No prep data</div>'; return; }

  const maxPortions = Math.max(...portions.map(p => p.portions_available || 0), 1);

  el.innerHTML = portions.map(p => {
    const pct = Math.min(100, Math.round(((p.portions_available || 0) / maxPortions) * 100));
...
function renderPrepList(portions) {
  ...
  el.innerHTML = portions.map(p => {
    ...
    const statusLabel = status === 'ok' ? 'OK' : status === 'low' ? 'Low' : 'Critical';
    return `
...
function renderEfficiencyChart(batches) {
  ...
  if (!batches.length) { el.innerHTML = '<div class="loading-row">No batch data</div>'; return; }

  const reversed = [...batches].reverse();
  const maxH = 110; // px height for 100%

  const bars = reversed.map(b => {
    const eff = parseFloat(b.efficiency_pct) || 0;
    const h   = Math.max(4, Math.round((eff / 100) * maxH));
    const color = eff >= 90 ? 'var(--green)' : eff >= 70 ? 'var(--amber)' : 'var(--red)';
    const name = b.items?.item_name || b.item_id;
    const shortName = name.replace('Mashed Potato','Mashed').replace('Steak Fries','S.Fries')
                         .replace('Beef Patty','Patty').replace('Mixed Salad','Salad')
                         .replace('Cajun Fries','C.Fries').replace('Honey Lime','H.Lime');
    return `
      <div class="bar-group">
        <div class="bar-fill"
...
function renderAlertList(stock) {
  const alerts = stock.filter(s => s.stock_status !== 'ok')
...
async function loadProduction() {
  ...
  sel.addEventListener('change', async () => {
    ...
    if (!itemId) { document.getElementById('prod-std').value = ''; return; }
    const std = await fetchBatchStandard(itemId);
    _batchStandards[itemId] = std;
    const intended = parseFloat(document.getElementById('prod-intended').value) || 1;
  ...
  document.getElementById('prod-intended').addEventListener('input', () => {
    ...
    const intended = parseFloat(document.getElementById('prod-intended').value) || 1;
    if (std) document.getElementById('prod-std').value = (std.standard_yield_gram * intended) + ' gr';
    recalcEfficiency();
  });

  document.getElementById('prod-yield').addEventListener('input', recalcEfficiency);
...
function recalcEfficiency() {
  ...
  if (!std || !actual) { document.getElementById('prod-eff').value = ''; return; }
...
async function handleProdSubmit(e) {
  ...
  if (!std) { fb.textContent = 'Select a valid item first'; fb.className='form-feedback error'; btn.disabled=false; return; }
...
function renderProdTable(batches) {
  ...
  if (!batches.length) { tbody.innerHTML = '<tr><td colspan="8" class="loading-row">No batches yet</td></tr>'; return; }

  tbody.innerHTML = batches.map(b => {
    const eff = parseFloat(b.efficiency_pct) || 0;
    const effClass = eff >= 90 ? 'eff-good' : eff >= 70 ? 'eff-warn' : 'eff-bad';
    return `
      <tr>
...
function filterStock() {
  ...
  const filtered = _allStock.filter(s =>
    (!cat    || s.category    === cat) &&
...
function renderStockTable(stock) {
  ...
  if (!stock.length) { tbody.innerHTML = '<tr><td colspan="6" class="loading-row">No items</td></tr>'; return; }
...
async function loadPurchases() {
  ...
  itemSel.innerHTML = '<option value="">— select ingredient —</option>' +
    ingredients.map(i => `<option value="${i.item_id}" data-conv="${i.conversion}" data-pu="${i.purchase_unit}">${i.item_name} (${i.purchase_unit})</option>`).join('');
  ...
  const recalcPO = () => {
    ...
    document.getElementById('po-qty-usage').value = qty
      ? fmtNum(qty * conv) + ' ' + (pu === 'kg' ? 'gr' : pu === 'ltr' ? 'ml' : 'pcs')
      : '';
    document.getElementById('po-total').value = qty && price
...
function renderPOTable(history) {
  ...
  if (!history.length) { tbody.innerHTML = '<tr><td colspan="8" class="loading-row">No purchases yet</td></tr>'; return; }
...
function fmtNum(n) {
  ...
  return isNaN(num) ? '—' : num.toLocaleString('id-ID', { maximumFractionDigits: 1 });
}

function fmtDate(d) {
  if (!d) return '—';
  return new Date(d).toLocaleDateString('en-ID', { day:'2-digit', month:'short', year:'numeric' });
}
...
async function loadRecipes() {
  ...
  document.getElementById('rtab-existing').onclick = () => switchRTab('existing');

  // Populate existing item selector
  const existingSel = document.getElementById('ri-existing-sel');
  existingSel.innerHTML = '<option value="">— select —</option>' +
    prepDishItems.map(i => `<option value="${i.item_id}">${i.item_name} (${i.category})</option>`).join('');

  // Populate recipe input items dropdown
  const inputSel = document.getElementById('rl-input-item');
  inputSel.innerHTML = '<option value="">— select ingredient or prep —</option>' +
    allInputItems.map(i =>
...
function switchRTab(tab) {
  ...
  document.getElementById('rtab-existing').classList.toggle('active', tab === 'existing');
  document.getElementById('rpanel-new').style.display      = tab === 'new'      ? 'flex' : 'none';
  document.getElementById('rpanel-existing').style.display = tab === 'existing' ? 'flex' : 'none';
}
...
async function handleCreateItem() {
  ...
  if (!name) { fb.textContent = 'Item name is required'; fb.className = 'form-feedback error'; return; }
  ...
  const prefix  = cat === 'Ingredient' ? 'ING' : cat === 'Prep' ? 'PREP' : 'DISH';
...
function activateRecipeEditor(item, existingLines) {
  ...
  document.getElementById('recipe-std-card').style.display   = item.category === 'Dish' ? 'none' : 'block';
...
function handleAddRecipeLine() {
  ...
  const name = sel.selectedOptions[0]?.text.replace(/^\[.*?\]\s*/, '') || inputId;
  ...
  if (_recipeLines.find(l => l.input_item_id === inputId)) {
    ...
    _recipeLines.push({ input_item_id: inputId, qty, unit, items: { item_name: name } });
  }
...
function renderRecipeLineTable() {
  ...
  if (!_recipeLines.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="loading-row">No lines yet — add ingredients above</td></tr>';
    return;
  }
  tbody.innerHTML = _recipeLines.map((l, idx) => `
    <tr>
...
async function handleSaveRecipe() {
  ...
  if (!_activeRecipeItemId) { fb.textContent = 'No item selected'; fb.className = 'form-feedback error'; return; }
  if (!_recipeLines.length) { fb.textContent = 'Add at least one ingredient'; fb.className = 'form-feedback error'; return; }
...
function recalcPortions() {
  ...
  document.getElementById('std-portions-calc').value = (y && p) ? (y / p).toFixed(1) + ' portions' : '';
}
...
async function handleSaveStandard(e) {
  ...
  const { error } = await saveBatchStandard(_activeRecipeItemId, stdYield, portion);
...
function renderRecipeLibrary(library) {
  ...
  if (!library.length) { el.innerHTML = '<div class="loading-row">No recipes yet</div>'; return; }

  el.innerHTML = library.map(item => {
    ...
    const linesHtml = item.lines.length ? `
      ...
      </table>` : '<div class="loading-row" style="padding:12px 0">No recipe lines</div>';
...
async function loadItems() {
  ...
  document.getElementById('items-filter-cat').onchange = () => {
    ...
    renderItemsTable(cat ? _allItemsMaster.filter(i => i.category === cat) : _allItemsMaster);
  };
...
async function handleItemSubmit(e) {
  ...
  const item = {
    item_id:       _editingItemId || document.getElementById('item-id').value.trim().toUpperCase(),
    ...
    conversion:    parseFloat(document.getElementById('item-conversion').value) || 1,
    par_level:     parseFloat(document.getElementById('item-par').value) || 0,
    is_active:     true,
  };

  if (!item.item_id) { fb.textContent = 'Item ID is required'; fb.className = 'form-feedback error'; btn.disabled = false; return; }
...
function populateItemForm(item) {
  ...
  document.getElementById('item-purchase-unit').value = item.purchase_unit || '';
  ...
  document.getElementById('item-conv-note').value =
    item.purchase_unit ? `1 ${item.purchase_unit} = ${item.conversion} ${item.usage_unit}` : '';
  document.getElementById('item-form-btn').scrollIntoView({ behavior:'smooth', block:'center' });
}
...
function renderItemsTable(items) {
  ...
  if (!items.length) { tbody.innerHTML = '<tr><td colspan="8" class="loading-row">No items</td></tr>'; return; }
...
async function handleSupplierSubmit(e) {
  ...
  const sup = {
    supplier_id:   _editingSupplierId || document.getElementById('sup-id').value.trim().toUpperCase(),
  ...
  if (!sup.supplier_id) { fb.textContent = 'Supplier ID is required'; fb.className = 'form-feedback error'; btn.disabled = false; return; }
  ...
  if (error) {
    ...
    fb.textContent = _editingSupplierId ? '✓ Supplier updated' : '✓ Supplier added';
    fb.className = 'form-feedback';
    cancelSupplierEdit();
    const suppliers = await fetchAllSuppliers();
    renderSuppliersTable(suppliers);
...
function populateSupplierForm(sup) {
  ...
  document.getElementById('sup-form-btn').scrollIntoView({ behavior:'smooth', block:'center' });
}
...
function renderSuppliersTable(suppliers) {
  ...
  if (!suppliers.length) { tbody.innerHTML = '<tr><td colspan="7" class="loading-row">No suppliers</td></tr>'; return; }
...
async function initOpeningStock() {
  ...
  sel.innerHTML = '<option value="">— select item —</option>' +
    allItems.map(i => `<option value="${i.item_id}" data-unit="${i.usage_unit}">${i.item_name} (${i.usage_unit})</option>`).join('');
...
function renderOsLines() {
  ...
  el.innerHTML = _osLines.map((l, idx) => `
...
async function handleSaveOpeningStock() {
  ...
  if (!_osLines.length) { fb.textContent = 'Add at least one item'; fb.className = 'form-feedback error'; return; }

  const ref  = document.getElementById('os-ref').value.trim();
  const date = document.getElementById('os-date').value;
  if (!ref || !date) { fb.textContent = 'Reference and date are required'; fb.className = 'form-feedback error'; return; }
...
async function loadService() {
  ...
  sel.innerHTML = '<option value="">— select dish —</option>' +
    dishes.map(d => `<option value="${d.item_id}">${d.item_name}</option>`).join('');
...
async function handleSvcAddDish() {
  ...
  if (existing) {
    ...
    _svcLines.push({ dishId, dishName, qty, unitPrice: price, recipe: _svcRecipes[dishId] || [] });
  }
...
function renderSvcLines() {
  ...
  if (!_svcLines.length) { el.innerHTML = ''; return; }

  el.innerHTML = _svcLines.map((l, idx) => {
...
function renderSvcPreview() {
  ...
  if (!_svcLines.length) { preview.style.display = 'none'; return; }
  ...
  if (!rows.length) { tbody.innerHTML = '<tr><td colspan="4" class="loading-row">No recipe lines found</td></tr>'; return; }
...
async function handleSvcSave() {
  ...
  if (!date)          { fb.textContent = 'Date is required'; fb.className = 'form-feedback error'; return; }
  if (!_svcLines.length) { fb.textContent = 'Add at least one dish'; fb.className = 'form-feedback error'; return; }
...
function renderSalesTable(history) {
  ...
  if (!history.length) { tbody.innerHTML = '<tr><td colspan="6" class="loading-row">No sales recorded yet</td></tr>'; return; }
...
async function loadAdjustments() {
  ...
  sel.onchange = async () => {
    ...
    document.getElementById('adj-system-qty').value = fmtNum(sysQty) + ' ' + (sel.selectedOptions[0]?.dataset.unit || '');
    sel._sysQty = sysQty;
    recalcDelta();
  };
...
function recalcDelta() {
  ...
  if (sysQty === null || isNaN(physical)) { document.getElementById('adj-delta').value = ''; return; }
...
async function handleAdjSave() {
  ...
  if (!itemId || !date || isNaN(physical) || !reason) {
    fb.textContent = 'All fields are required'; fb.className = 'form-feedback error'; return;
  }
...
function renderAdjTable(history) {
  ...
  if (!history.length) { tbody.innerHTML = '<tr><td colspan="7" class="loading-row">No adjustments yet</td></tr>'; return; }
...
function filterLedger() {
  ...
  const filtered = _allLedger.filter(r =>
...
function renderLedgerTable(rows) {
  ...
  if (!rows.length) { tbody.innerHTML = '<tr><td colspan="10" class="loading-row">No movements found</td></tr>'; return; }
  ...
  tbody.innerHTML = rows.map(r => {
    ...
    const netCol = net > 0 ? 'var(--green)' : net < 0 ? 'var(--red)' : 'var(--text-muted)';
    const sign   = net > 0 ? '+' : '';
...
async function loadCosting() {
  ...
  document.getElementById('cost-filter-cat').onchange = () => {
    ...
    renderRecipeCostTable(cat ? _allRecipeCosts.filter(r => r.category === cat) : _allRecipeCosts);
  };

  document.getElementById('ing-cost-filter').onchange = () => {
    const src = document.getElementById('ing-cost-filter').value;
    renderIngCostTable(src ? _allIngCosts.filter(i => i.cost_source === src) : _allIngCosts);
  };
...
function renderRecipeCostTable(costs) {
  ...
  if (!costs.length) { tbody.innerHTML = '<tr><td colspan="7" class="loading-row">No recipes</td></tr>'; return; }

  tbody.innerHTML = costs.map(r => {
    const hasWarning = r.has_uncosted_ingredients;
    const costDisplay = r.cost_per_portion != null
      ? 'Rp ' + Math.round(r.cost_per_portion).toLocaleString('id-ID')
      : '—';
    const batchDisplay = r.total_batch_cost != null
    ...
    return `<tr style="cursor:pointer" onclick="showCostBreakdown('${r.output_item_id}','${r.recipe_name}',${r.portions_per_batch})">
      ...
      <td>${hasWarning
        ...
        : '<span class="pill pill-ok" style="font-size:9px">✓ Complete</span>'}</td>
...
async function showCostBreakdown(itemId, name, portionsPerBatch) {
  ...
  document.getElementById('cost-breakdown-title').textContent = name + ' — Cost Breakdown';
  document.getElementById('cost-breakdown-sub').textContent   = `${portionsPerBatch} portions per batch`;
  document.getElementById('cost-breakdown-tbody').innerHTML   = '<tr><td colspan="7" class="loading-row">Loading…</td></tr>';
  card.scrollIntoView({ behavior:'smooth', block:'start' });

  const lines = await fetchRecipeCostLines(itemId);

  if (!lines.length) {
    document.getElementById('cost-breakdown-tbody').innerHTML = '<tr><td colspan="7" class="loading-row">No recipe lines</td></tr>';
    return;
  }

  const srcBadge = src => {
    const colors = { purchase:'var(--green)', override:'var(--amber)', uncosted:'var(--red)' };
  ...
  document.getElementById('cost-breakdown-total').textContent       = 'Rp ' + Math.round(total).toLocaleString('id-ID');
  document.getElementById('cost-breakdown-per-portion').textContent = 'Rp ' + Math.round(perPortion).toLocaleString('id-ID');
}

function renderIngCostTable(costs) {
  const tbody = document.getElementById('ing-cost-tbody');
  if (!costs.length) { tbody.innerHTML = '<tr><td colspan="7" class="loading-row">No items</td></tr>'; return; }
  ...
  tbody.innerHTML = costs.map(i => `
    <tr>
      ...
      <td class="mono" style="font-weight:600;color:var(--text-primary)">${i.resolved_cost != null ? 'Rp ' + fmtNum(i.resolved_cost) : <span style="color:var(--red)">Uncosted</span>}</td>
...
function openOverrideEditor(itemId, name, unit, currentOverride, currentNote) {
  ...
  document.getElementById('override-editor').scrollIntoView({ behavior:'smooth', block:'start' });
}
...
async function handleOverrideSave() {
  ...
  if (isNaN(cost) || cost < 0) { fb.textContent = 'Enter a valid cost'; fb.className = 'form-feedback error'; return; }

  fb.textContent = 'Saving…'; fb.className = 'form-feedback';
  const { error } = await updateCostOverride(_overrideItemId, cost, note);
  if (error) { fb.textContent = 'Error: ' + error; fb.className = 'form-feedback error'; return; }
...
async function handleOverrideClear() {
  const fb = document.getElementById('override-feedback');
  fb.textContent = 'Clearing…'; fb.className = 'form-feedback';
...
async function loadCalculator() {
  ...
  itemSel.innerHTML = '<option value="__custom__">+ Custom ingredient</option>' +
    _calcAllItems.map(i => `<option value="${i.item_id}" data-unit="${i.usage_unit}">${i.item_name} (${i.usage_unit})</option>`).join('');

  itemSel.onchange = () => {
    const isCustom = itemSel.value === '__custom__';
    document.getElementById('calc-custom-name').style.display = isCustom ? 'block' : 'none';
    if (!isCustom) {
...
function renderCalcSessionList(sessions) {
  ...
  if (!sessions.length) {
    el.innerHTML = '<div class="loading-row">No sessions yet — create one</div>';
    return;
  }
...
async function openCalcSession(sessionId, name) {
  ...
  _calcLines = lines.map(l => ({
    ...
    name:        l.custom_name || (_calcAllItems.find(i => i.item_id === l.item_id)?.item_name) || l.item_id,
    qty:         l.qty,
    unit:        l.unit,
...
function renderCalcLines() {
  ...
  if (!_calcLines.length) { el.innerHTML = ''; return; }
  el.innerHTML = _calcLines.map((l, idx) => `
...
function recalcCalcSummary() {
  ...
  const portions = parseFloat(document.getElementById('calc-portions').value) || 1;
  const perPortion = total / portions;
  const suggested  = perPortion / 0.30;   // 30% food cost target

  document.getElementById('calc-total').textContent         = 'Rp ' + Math.round(total).toLocaleString('id-ID');
  document.getElementById('calc-per-portion').value         = 'Rp ' + Math.round(perPortion).toLocaleString('id-ID');
  document.getElementById('calc-suggested-price').value     = 'Rp ' + Math.round(suggested).toLocaleString('id-ID');
}
...
async function handleCalcSave() {
  ...
  if (error) { fb.textContent = 'Error: ' + error; fb.className = 'form-feedback error'; }
  else { fb.textContent = '✓ Saved'; fb.className = 'form-feedback'; }
  btn.disabled = false;
}
